...
/*this class is used to load .obj 3D models with Assimp (open asset import library)   */
    ...
    private static List<Model.Animation> processAnimations(AIScene aiScene, List<Bone> boneList,
        ...
        for (int i = 0; i < numAnimations; i++) {
            ...
            for (int j = 0; j < maxFrames; j++) {
                Matrix4f[] boneMatrices = new Matrix4f[Consts.MAX_BONES];
                Arrays.fill(boneMatrices, Consts.IDENTITY_MATRIX);
    ...
    */
    ...
    private static AnimMeshData processBones(AIMesh aiMesh, List<Bone> boneList){
        ...
        for (int i = 0; i < numVertices; i++) {
            ...
            for (int j = 0; j < Consts.MAX_WEIGHTS; j++) {
                if (j < size) {
