...
/*
 * The code here is really long but in short
 * we set up lots of attributes to get
 * the values from the GUI controls and a method
 * to draw the panels and "widgets" required
 * for controlling lights in game
 * this will should be used only in dev phase
 * later, it should be change to a proper ingame implementation
 */
...
public class LightControls implements IGuiInstance {
    ...
    /*
     * handle GUI input, update Imgui based on mouse status and check if the input
     * has been consumed by the GUI controls, if so, populate the attributes of the class
